...
impl MockExchangeFeedSimulator {
    ...
    pub fn generate_burst(
        ...
        for i in 0..count {
            ...
            let jitter_ns = self
                .prng
                .gen_range(config.jitter_min_ns, config.jitter_max_ns);
            ...
            let event = if venue_choice < 5 {
                ...
                if is_trade {
                    ...
                    let side = if (self.prng.next_u64() & 1) == 0 {
                        1
                    } else {
                        2
                    };
...
mod tests {
    ...
    fn test_feed_generator_burst() {
        ...
        assert!(
            first.venue_id == VenueId::Binance as u16
    ...
    fn test_saturation_policy_drop_newest() {
        ...
        assert_eq!(
            queue.push_with_policy(1, SaturationDropPolicy::DropNewest),
            PushResult::Enqueued
        );
        assert_eq!(
            queue.push_with_policy(2, SaturationDropPolicy::DropNewest),
            PushResult::Enqueued
        );
        assert_eq!(
            queue.push_with_policy(3, SaturationDropPolicy::DropNewest),
            PushResult::Enqueued
        );
        assert_eq!(
            queue.push_with_policy(4, SaturationDropPolicy::DropNewest),
            PushResult::Enqueued
        );

        // 5th item: buffer full, should drop newest
        assert_eq!(
            queue.push_with_policy(5, SaturationDropPolicy::DropNewest),
            PushResult::DroppedNewest
        );
    ...
    fn test_saturation_policy_drop_oldest() {
        ...
        assert_eq!(
            queue.push_with_policy(10, SaturationDropPolicy::DropOldest),
            PushResult::Enqueued
        );
        assert_eq!(
            queue.push_with_policy(20, SaturationDropPolicy::DropOldest),
            PushResult::Enqueued
        );
        assert_eq!(
            queue.push_with_policy(30, SaturationDropPolicy::DropOldest),
            PushResult::Enqueued
        );
        assert_eq!(
            queue.push_with_policy(40, SaturationDropPolicy::DropOldest),
            PushResult::Enqueued
        );

        // 5th item: buffer full, should evict oldest (10) and write 50
        assert_eq!(
            queue.push_with_policy(50, SaturationDropPolicy::DropOldest),
            PushResult::EvictedOldest
        );
        ...
        assert_eq!(
            queue.push_with_policy(60, SaturationDropPolicy::DropOldest),
            PushResult::EvictedOldest
        );
